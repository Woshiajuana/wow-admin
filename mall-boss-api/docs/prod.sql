...
/*数据库名更改请批量替换  当前数据库名:score_mall */

/*app名称 当前:积分商城管理平台 */

/*app域名更改请替换 当前域名:mall-boss.ptjxd.com */

/*初始化operator账户信息  账户 admin  密码 admin1! */

/*admin密码替换 当前:8cb08622a46d5800f0f332fb55b8c553  密码规则:md5加密*/

/*logo地址 当前:https://assetobjs.oss-cn-shanghai.aliyuncs.com/logo/logo_mall.png */

/*app登陆配置 搜索关键词修改
当前:
  isImageCaptcha-0 关闭登陆图形验证码
  isSms-0 关闭手机短信验证 （未支持）
  isOtp-1 开启动态口令配置，无则直接登陆进去，推荐绑定
*/


/*创建数据库*/
drop database if exists score_mall;
create database if not exists score_mall default character set utf8mb4 collate utf8mb4_general_ci;

/**************************初始化表结构****************************/

/*初始化 组织 */
drop table if exists score_mall.pp_org;
CREATE TABLE score_mall.pp_org
(
...
/*初始化 APP */
drop table if exists score_mall.pp_org_app;
CREATE TABLE score_mall.pp_org_app
(
  id            bigint(20)  NOT NULL AUTO_INCREMENT,
...
/*初始化 APP域名 */
drop table if exists score_mall.pp_org_app_domain;
CREATE TABLE score_mall.pp_org_app_domain
(
  id         bigint(20)  NOT NULL AUTO_INCREMENT,
...
/*初始化 组织配置 */
drop table if exists score_mall.pp_org_config;
CREATE TABLE score_mall.pp_org_config
(
  id            bigint(20)  NOT NULL AUTO_INCREMENT,
  org_no        varchar(30) NOT NULL COMMENT '组织号 全局唯一， 全局第一个组织是根,规则 固定 XX000000000000000000 20位',
...
/*初始化 app配置 */
drop table if exists score_mall.pp_app_config;
CREATE TABLE score_mall.pp_app_config
(
  id            bigint(20)  NOT NULL auto_increment,
...
/*初始化 操作员 */
drop table if exists score_mall.pp_operator;
CREATE TABLE score_mall.pp_operator
(
  id                  bigint(20)  NOT NULL AUTO_INCREMENT,
...
/*初始化 角色 */
drop table if exists score_mall.pp_role;
CREATE TABLE score_mall.pp_role
(
  id         bigint(20)  NOT NULL AUTO_INCREMENT,
...
/*初始化 菜单资源 */
drop table if exists score_mall.pp_menu_resource;
CREATE table if not exists score_mall.pp_menu_resource (
  id bigint(20) NOT NULL AUTO_INCREMENT,
...
/*初始化 角色权限 */
drop table if exists score_mall.pp_role_permission;
CREATE TABLE score_mall.pp_role_permission
(
  id            bigint(20)  NOT NULL AUTO_INCREMENT,
...
/*初始化 操作日志 */
drop table if exists score_mall.pp_operation_log;
CREATE table if not exists score_mall.pp_operation_log
(
    id bigint(20)  NOT NULL AUTO_INCREMENT,
    app_no varchar(30) NOT NULL COMMENT 'app no',
...
/*初始化org app*/
INSERT INTO `score_mall`.`pp_org_app`
( `app_no`,`org_no`, `belong_to_org`, `category`, `app_code`, `type`, `name`, `logo`, `desc`,`theme`, `icp`, `app_info`)
VALUES
('AP000000001611384705','PL000000001805016656', 'PL000000001805016656', 'root','boss','boss','积分商城管理平台', 'https://assetobjs.oss-cn-shanghai.aliyuncs.com/logo/logo_mall.png', 'desc..', 'default', '', '{}' ); /*这里配置app信息和名称，logo是URL*/

/*配置logo地址*/
-- UPDATE score_mall.pp_org_app set logo = 'https://assetobjs.oss-cn-shanghai.aliyuncs.com/logo/logo_mall.png' where app_no = 'AP000000001611384705' ;

/*初始化org app domain*/
INSERT INTO `score_mall`.`pp_org_app_domain`
(`app_no`, `domain`)
VALUES
('AP000000001611384705', 'localhost:8080'), /*这里配置域名映射，可配置多条*/
('AP000000001611384705', 'mall-boss.ptjxd.com'); /*app使用的域名*/


/*初始化app config*/
INSERT INTO `score_mall`.`pp_app_config`
(`app_no`, `category`, `key`, `value`)
VALUES

('AP000000001611384705', 'system', 'isImageCaptcha', '0'), /*是否开启图形验证码验证*/
('AP000000001611384705', 'system', 'isSms', '0'), /*是否开启手机验证*/
('AP000000001611384705', 'system', 'isOtp', '1'); /*是否开启动态口令验证*/


/*初始化operator  账户 admin  密码 admin1! */
INSERT INTO `score_mall`.`pp_operator`
...
/*初始化role*/
INSERT INTO `score_mall`.`pp_role`( `role_no`, `parent`, `org_no`, `app_no`, `name`, `desc`, `level`, `status`, `creator_name`) 
...
/*根菜单*/
INSERT INTO `score_mall`.`pp_menu_resource`
...
/*基础权限*/
INSERT INTO `score_mall`.`pp_role_permission`
